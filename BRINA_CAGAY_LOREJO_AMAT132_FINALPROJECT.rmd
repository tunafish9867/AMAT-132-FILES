...
```{r, fig.cap='Time Series Plot of the Forecasted Purchasing Power of Peso in Davao City from May 2023 to December 2024', fig.height=3, fig.width=8, fig.align='center'}
fit1 <- Arima(y=purch_pow.ts, order = c(0,1,1))
fit2 <- Arima(y=purch_pow.ts, order = c(1,1,0))
fit3 <- auto.arima(y=purch_pow.ts, seasonal = FALSE)
fit4 <- auto.arima(y=purch_pow.ts, stepwise = FALSE, approximation = FALSE, seasonal = FALSE)
...
In forecasting the data, the authors have shown the four ARIMA models to forecast the next 19 months or from June 2023 to December 2024, and also predicted with a 95% confidence interval to provide a better range of outcomes, the forecast shows that the purchasing power of peso in Davao City will continue to decrease or inhibits a downward trend for both auto.arima models. Hence, the trend follows its typical pattern, as the previous data's behavior shows. On the other hand, the forecast is stationary for ARIMA(0,1,1) and ARIMA(1,1,0).
\newpage

## CONCLUSION

Based on the study's findings, the authors employed the four models to conduct a comprehensive forecast for June 2023 to December 2024, encompassing the next 19 months. A 95% confidence interval was also utilized to provide a range of potential outcomes.

Based on the auto.arima models, the forecasted results indicate a continued decline in the purchasing power of peso in Davao City. This finding aligns with the observed historical data, suggesting that the trend follows a consistent pattern. In contrast, the ARIMA models showed a stationary forecast, implying that the purchasing power will be the same until December 2024. 

However, it should be noted that the auto.arima models are not feasible in real-life since purchasing power should not decrease until zero. Hence, the authors recommend, to further enhance the paper, to apply another forecasting method, such as damped trend method of forecasting. Nonetheless, these results hold valuable implications for various stakeholders.
