...
diff0<-purch_pow.ts %>% ur.kpss() %>% summary()
diff1 <- diff(purch_pow.ts, differences = 1)
diff2<-diff1 %>% ur.kpss() %>% summary()
acfplot1=ggAcf(diff1) + ggtitle("ACF Plot")
acfplot2=ggPacf(diff1) + ggtitle("PACF Plot")
...
kable(AICC, caption = "The RMSE values of the four (4) ARIMA models", booktabs = T) %>%
    kable_styling(latex_options = "hold_position") %>%
    kable_styling(latex_options = "striped")
```
